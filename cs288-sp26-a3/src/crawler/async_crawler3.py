...
OUTPUT_FILE = os.path.join(OUTPUT_DIR, "corpus_raw3.jsonl")
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            async with session.get(url, timeout=timeout, allow_redirects=True) as response:
                ...
                ClientPayloadError, asyncio.TimeoutError, ConnectionResetError) as e:
            ...
            if attempt == MAX_RETRIES - 1 and url.startswith("https://"):
                ...
                try:
                    async with session.get(http_url, timeout=timeout, allow_redirects=True) as response:
                        return response
    ...
    async with semaphore:
        try:
            ...
            if response.status == 403:
                logging.warning(f"403 WAF triggered on {url}. Cooling down {COOLDOWN_ON_403}s...")
...
if __name__ == "__main__":
    ...
    asyncio.run(main(max_pages=5000))