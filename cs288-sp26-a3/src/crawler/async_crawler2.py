...
OUTPUT_FILE = os.path.join(OUTPUT_DIR, "corpus_raw2.jsonl")
...
async def fetch_and_parse(session, url, semaphore):
    async with semaphore:
        try:
            ...
            # CRITICAL FIX: ssl=False. University subdomains often have weird certs that instantly crash bots!
            async with session.get(url, timeout=20, allow_redirects=True, ssl=False) as response:
                if response.status == 403:
                    logging.warning(f"403 WAF Triggered on {url}. Cooling down for {COOLDOWN_ON_403}s...")
            ...
            # CRITICAL FIX: Explicitly log the error so we can see why it failed instead of crashing silently
    ...
    while True:
        try:
            ...
            if page_data and len(page_data['content']) > 50:
                ...
                # Print progress every 5 pages so you can see it working quickly
                if stats['saved'] % 5 == 0:
...
async def main(max_pages=20000):
    ...
    # FAILSAFE FIX: If the queue is empty because the seeds were already in the visited file,
    # force them into the queue anyway so the script actually runs!
    if queue.empty():
        logging.info("Seeds already in visited list. Forcing them into the queue to dig deeper...")
        for url in seed_urls:
            queue.put_nowait(url)
