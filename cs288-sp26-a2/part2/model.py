...
class MultiHeadSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        mask = self._create_causal_mask(x.shape[-2], x.device)
...
def count_flops_per_token(
    ...
    QK = 2 * context_length * d_model

    attn = 2 * context_length * d_model

    ffn = 2 * 3 * d_model * d_ff

    LMhead = 2 * d_model * vocab_size

    return (QKV + output + QK + attn + ffn) * num_layers + LMhead
...
def estimate_memory_bytes(
    ...
    perlayer_rms = 2 * d_model

    qkv = 4 * d_model ** 2

    ffn = 3 * d_model * d_ff

    return dtype_bytes * (token_embeddings + output + rms + perlayer_rms + qkv + ffn)