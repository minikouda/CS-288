        ...
        while True:
            ...
            for i in range(len(tokens) - 1):
                ...
                if merged in self.inverse_vocab:
                    ...
                    if rank < lowest_rank:
                        lowest_rank = rank
                        lowest_pair = (tokens[i], tokens[i + 1])
            ...
            while i < len(tokens) - 1:
                if tokens[i] == lowest_pair[0] and tokens[i + 1] == lowest_pair[1]:
                    new_tokens.append(lowest_pair[0] + lowest_pair[1])
                    i += 2
                else:
        ...
        for pre_token in self.pat.findall(text):
            ...
            for token in tokens:
                if token in self.inverse_vocab:
                    ...
                    for t in token:
                        if bytes([t]) in self.inverse_vocab:
                            ids.append(self.inverse_vocab[bytes(t)])
