...
def train_bpe(
    ...
    for pre_token in pre_tokens:
        ...
        for fstring in forbidden_substrings:
            if fstring in token_bytes:
                break
        else:
    ...
    while len(vocab) < vocab_size:
        ...
        # 4a: highest freq wins; ties broken by lexicographically smallest pair
        # best_pair = max(pair_counts, key=lambda p: (pair_counts[p], [-b for b in p[0] + p[1]]))
        best_pair = max(pair_counts, key=lambda p: (pair_counts[p], p))
        new_token = best_pair[0] + best_pair[1]

        # 4b: add merged token to vocab and record the merge
        vocab[len(vocab)] = new_token
        merges.append(best_pair)

        # 4c & 4d: update word_freqs and pair_counts
        new_word_freqs = Counter()
        for word, freqs in word_freqs.items():
            # Check if the best_pair appears adjacently in this word
            if any(word[i] == best_pair[0] and word[i+1] == best_pair[1]
                   for i in range(len(word) - 1)):
                # Subtract old pairs from pair_counts
                for old_pair in get_pairs(word):
                    pair_counts[old_pair] -= freqs
                    if pair_counts[old_pair] == 0:
                        del pair_counts[old_pair]
                # Merge the word and add new pairs
                merged_word = merge_word(word, best_pair)
                new_word_freqs[merged_word] += freqs
                for new_pair in get_pairs(merged_word):
                    pair_counts[new_pair] += freqs
            else:
                new_word_freqs[word] += freqs
